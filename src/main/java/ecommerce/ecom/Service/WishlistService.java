    ...
    public ResponseEntity<CommonDTO> addToWishlist(WishlistAddDTO wishDto) {
        try {
            ...
            if (serviceOptional.isPresent() && !userList.isEmpty()) {
                ...
                return new ResponseEntity<>(new CommonDTO("ADD WISHLIST", "ERROR", "User or service not found"),
                        HttpStatus.NOT_FOUND);
            }
        } catch (Exception e) {
            return new ResponseEntity<>(new CommonDTO("ADD WISHLIST", "ERROR", "Something went wrong.."),
    ...
    public ResponseEntity<?> getAllWishlist(String userId) {
        try {
            ...
            if (!wl_raw.isEmpty()) {
                ...
                return new ResponseEntity<>(new CommonDTO("GET WISHLIST", "ERROR", "No items found"),
                        HttpStatus.NOT_FOUND);
            }
        } catch (Exception e) {
            return new ResponseEntity<>(new CommonDTO("GET WISHLIST", "ERROR", "Something went wrong.."),
                    HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }

    public ResponseEntity<?> removeWish(String wishId) {
        try {
            Optional<Wishlist> wlItem = wishlistRepository.findById(wishId);

            if (wlItem.isEmpty()) {
                return ResponseEntity
                        .status(404)
                        .body("Wishlist item not found");
            }

            wishlistRepository.delete(wlItem.get());

            return ResponseEntity
                    .status(200)
                    .body("Removed from wishlist successfully");
